@echo off
REM Build script for ChipAnchor: The Little UNIVAC AES Encryptor
REM Unified script for MSVC and MinGW compilation
REM Supports Windows and UNIVAC cross-compilation

echo.
echo ========================================
echo   ChipAnchor - Unified Build Script
echo   The Little UNIVAC AES Encryptor
echo ========================================
echo.
...
:BUILD_START
echo.
echo Building ChipAnchor: The Little UNIVAC AES Encryptor...
echo.
...
if exist chipanchor_univac.exe del /Q chipanchor_univac.exe
if exist chipanchor_univac.o del /Q chipanchor_univac.o
if exist *.o del /Q *.o
echo.
...
echo Compiling chipanchor.c...
gcc -c -DUNIVAC -O2 -Wall chipanchor.c -o chipanchor_univac.o

if %ERRORLEVEL% NEQ 0 (
    echo ERROR: Failed to compile chipanchor.c
    pause
    exit /b 1
)

echo Linking...
gcc -o chipanchor_univac.exe chipanchor_univac.o
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ========================================
    echo.
    echo Output: chipanchor_univac.exe

    REM Display file size
    for %%A in (chipanchor_univac.exe) do (
...
REM Clean previous build artifacts
echo Cleaning previous build artifacts...
if exist chipanchor_mingw.exe del /Q chipanchor_mingw.exe
if exist chipanchor.obj del /Q chipanchor.obj
if exist *.o del /Q *.o
echo.
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ========================================
    echo.
    echo Output: chipanchor_mingw.exe

    REM Display file size
    for %%A in (chipanchor_mingw.exe) do (
        echo File size: %%~zA bytes
    )
    echo.
    echo Optimization level: MAXIMUM (-O3 + LTO + native CPU^)
    echo.
    echo To run ChipAnchor, type: chipanchor_mingw.exe
    echo.
) else (
    echo.
    echo ========================================
...
cl /W4 /O2 /Fe:chipanchor.exe chipanchor.c
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ========================================
    echo.
    echo Output: chipanchor.exe
    echo.
    echo To run: chipanchor.exe
) else (
    echo.
    echo ========================================
    echo   BUILD FAILED
